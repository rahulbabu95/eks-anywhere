...
func (c *upgradeManagementTestSetup) expectPrepareUpgradeManagement(err error) {
	c.clusterUpgrader.EXPECT().PrepareUpgrade(c.ctx,
		c.newClusterSpec,
		c.managementCluster.KubeconfigFile,
		c.managementCluster.KubeconfigFile,
	).Return(err)
}
...
// func (c *upgradeManagementTestSetup) expectUpgradeManagementCluster(managementCluster *types.Cluster) {
// 	calls := []*gomock.Call{
// 		c.expectPrepareUpgradeManagement(managementCluster),
// 		c.expectUpgradeManagementToReturn(managementCluster, nil),
...
func (c *upgradeManagementTestSetup) expectCleanupAfterUpgrade(err error) {
	c.clusterUpgrader.EXPECT().CleanupAfterUpgrade(c.ctx, c.newClusterSpec, c.managementCluster.KubeconfigFile, c.managementCluster.KubeconfigFile).Return(err)
}
...
func TestUpgradeManagementRunPrepareUpgradeFailed(t *testing.T) {
	os.Unsetenv(features.CheckpointEnabledEnvVar)
	features.ClearCache()
	t.Setenv(features.ExperimentalSelfManagedClusterUpgradeEnvVar, "true")
	test := newUpgradeManagementClusterTest(t)
	...
	test.expectPrepareUpgradeManagement(errors.New(""))
	test.expectSaveLogs()
	test.expectWriteCheckpointFile()
...
func TestUpgradeManagementRunUpgradeEKSAFailed(t *testing.T) {
	...
	test.expectBackupManagementFromCluster(nil)
	test.expectPauseCAPIWorkloadClusters(nil)
	test.expectPrepareUpgradeManagement(nil)
...
func TestUpgradeManagementRunResumeCAPIWorkloadFailed(t *testing.T) {
	...
	test.expectCleanupAfterUpgrade(nil)
