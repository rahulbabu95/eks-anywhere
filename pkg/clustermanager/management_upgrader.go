...
// ManagementUpgraderRetrier allows to use a custom retrier.
// func ManagementUpgraderRetrier(retrier retrier.Retrier) ManagementUpgraderOpt {
// 	return func(c *ManagementUpgrader) {
// 		c.retrier = retrier
// 	}
// }
...
func (c *ManagementUpgrader) UpgradeManagementCluster(ctx context.Context, managementCluster *types.Cluster) error {
	...
	err = c.upgradeClient.WaitForClusterCondition(ctx, managementCluster, c.retrier, string(v1alpha1.WorkersReadyConditon), c.controlPlaneWaitTimeout.String())
	if err != nil {
		return fmt.Errorf("waiting for management cluster control plane to be ready: %v", err)
	}
