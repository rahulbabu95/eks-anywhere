...
func TestCloudStackKubernetes122RedhatTo123UpgradeWithCheckpoint(t *testing.T) {
	...
	clusterOpts = append(clusterOpts, framework.WithClusterUpgrade(api.WithKubernetesVersion(v1alpha1.Kube123)), framework.ExpectFailure(true), provider.WithProviderUpgrade(framework.UpdateRedhatTemplate122Var()), framework.WithEnvVar(features.CheckpointEnabledEnvVar, "true"), framework.WithEnvVar(framework.CleanupVmsVar, "false"))
...
func TestVSphereKubernetes123RedhatUpgradeFromLatestMinorReleaseAlwaysNetworkPolicy(t *testing.T) {
	...
	runUpgradeFromReleaseFlow(
		...
		v1alpha1.Kube123,
		framework.WithClusterFiller(api.WithCiliumPolicyEnforcementMode(v1alpha1.CiliumPolicyModeAlways)),
		provider.WithProviderUpgrade(framework.UpdateRedhatTemplate123Var()), // Set the template so it doesn't get autoimported
	)
}
